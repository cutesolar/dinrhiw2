...
FPTree::FPTree(const TransactionSourceInterface* transactions, uint64_t minimum_support_threshold) :
  ...
  minimum_support_threshold( minimum_support_threshold )
{
  ...
  {
    for(unsigned long i=0;i<transactions->getNumberOf();i++){
      transactions->getData(i, transaction);
  ...
  for(unsigned long index=0;index<transactions->getNumberOf();index++){
    transactions->getData(index, transaction);
...
bool fptree_growth(const FPTree& fptree, std::set<Pattern>& p)
{
  if ( fptree.empty() ) { p = {}; return true; }
  
  if ( contains_single_path( fptree ) ) {
    // generate all possible combinations of the items in the tree
    
    std::set<Pattern>& single_path_patterns = p;
    ...
    std::set<Pattern>& multi_path_patterns = p;
	...
	for ( unsigned long i = 0; i < transformed_prefix_path_items_frequency; ++i ) {
	  conditional_fptree_transactions.push_back( transaction );
	}
      }

      const ConditionalTreeTransactions cond_source(conditional_fptree_transactions);
      
      // build the conditional fptree relative to the current item with the transactions just generated
      const FPTree conditional_fptree( &cond_source, fptree.minimum_support_threshold );
      
      // call recursively fptree_growth on the conditional fptree (empty fptree: no patterns)
      std::set<Pattern> conditional_patterns;
      fptree_growth( conditional_fptree, conditional_patterns );
